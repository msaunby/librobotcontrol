...
static void cmd_turn(srv_t *s, float duty, float deg)
{
    ...
    double start  = gyro_heading_get_deg(&s->heading);
    double target = wrap_angle(start + (double)deg);
    float left_duty  = (deg > 0.0f) ? -duty :  duty;
    float right_duty = (deg > 0.0f) ?  duty : -duty;
    motor_ramp_set_target(&s->motor_ramp, left_duty, right_duty);

    time_t t0 = time(NULL);
    while (!g_shutdown && (time(NULL) - t0) < TURN_TIMEOUT_SEC) {
        double err = wrap_angle(target - gyro_heading_get_deg(&s->heading));
        if (fabs(err) <= TURN_TOLERANCE_DEG) break;
        us_sleep(25000);
    }
    motor_ramp_brake(&s->motor_ramp);
    reseed_heading(s);
